...
public:
  ...
  using PickingQueryResultCallback = void (*)(PickingQueryResult const& result, PickingQuery* UTILS_NONNULL pq);
  ...
  void pick(uint32_t x, uint32_t y, T* UTILS_NONNULL instance, backend::CallbackHandler* UTILS_NULLABLE handler = nullptr) noexcept {
    PickingQuery& query = pick(
        x, y, [](PickingQueryResult const& result, PickingQuery* pq) { (static_cast<T*>(pq->storage[0])->*method)(result); }, handler);
   ...
   */
  template <typename T, void (T::*method)(PickingQueryResult const&)>
  void pick(uint32_t x, uint32_t y, T instance, backend::CallbackHandler* UTILS_NULLABLE handler = nullptr) noexcept {
   ...
   * @param handler   Handler to dispatch the callback or nullptr for the default handler.
   */
  template <typename T> void pick(uint32_t x, uint32_t y, T functor, backend::CallbackHandler* UTILS_NULLABLE handler = nullptr) noexcept {
    static_assert(sizeof(functor) <= sizeof(PickingQuery::storage), "functor too large");
    PickingQuery& query = pick(
        x, y, handler, (PickingQueryResultCallback)[](PickingQueryResult const& result, PickingQuery* pq) {
          T* const that = static_cast<T*>(reinterpret_cast<void*>(pq->storage));
   ...
   */
  PickingQuery& pick(uint32_t x, uint32_t y, backend::CallbackHandler* UTILS_NULLABLE handler,
