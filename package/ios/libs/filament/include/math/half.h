...
namespace math {
  ...
  template <unsigned S, unsigned E, unsigned M> class fp {
  ...
  public:
    ...
    TYPE bits{};
    static constexpr size_t getBitCount() noexcept {
      return S + E + M;
    }
  ...
  private:
    ...
    friend constexpr uint16_t getBits(half const& h) noexcept {
      return h.mBits.bits;
    }
...
template <> class numeric_limits<filament::math::half> {
public:
