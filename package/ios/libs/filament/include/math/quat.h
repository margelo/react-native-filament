...
namespace details {
  ...
  template <typename T>
  class MATH_EMPTY_BASES TQuaternion : public TVecAddOperators<TQuaternion, T>,
  ...
  public:
    ...
    template <typename A, typename B, typename C, typename D, typename = enable_if_arithmetic_t<A, B, C, D>>
    ...
    constexpr static TQuaternion MATH_PURE fromDirectedRotation(const TVec3<A>& from, const TVec3<B>& to) {
      ...
      return TQuaternion(s * T(0.5), rotationAxis.x / s, rotationAxis.y / s, rotationAxis.z / s);
    }
  };
