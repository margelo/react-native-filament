...
template <typename T> uint32_t linearToRGB_10_11_11_REV(const T& linear) {
...
template <typename T> inline filament::math::float4 linearToRGBM(const T& linear) {
...
template <typename T> inline filament::math::float3 RGBMtoLinear(const T& rgbm) {
...
template <typename T> inline filament::math::float3 linearTosRGB(const T& linear) {
  using filament::math::float3;
...
template <typename T> T sRGBToLinear(const T& sRGB);

template <> inline filament::math::float3 sRGBToLinear(const filament::math::float3& sRGB) {
  using filament::math::float3;
  constexpr float a = 0.055f;
  constexpr float a1 = 1.055f;
...
template <> inline filament::math::float4 sRGBToLinear(const filament::math::float4& sRGB) {
  using filament::math::float4;
  constexpr float a = 0.055f;
  constexpr float a1 = 1.055f;
  constexpr float p = 2.4f;
...
template <typename T> T linearToSRGB(const T& color);

template <> inline filament::math::float3 linearToSRGB(const filament::math::float3& color) {
  using filament::math::float3;
  float3 sRGBColor{color};
  UTILS_NOUNROLL
  for (size_t i = 0; i < sRGBColor.size(); i++) {
    sRGBColor[i] = (sRGBColor[i] <= 0.0031308f) ? sRGBColor[i] * 12.92f : (powf(sRGBColor[i], 1.0f / 2.4f) * 1.055f) - 0.055f;
  }
  return sRGBColor;
}
...
template <typename T, int N = 3> std::unique_ptr<uint8_t[]> fromLinearTosRGB(const LinearImage& image) {
...
template <typename T, int N = 3> std::unique_ptr<uint8_t[]> fromLinearToRGB(const LinearImage& image) {
...
template <typename T> std::unique_ptr<uint8_t[]> fromLinearToRGBM(const LinearImage& image) {
...
template <typename T> std::unique_ptr<uint8_t[]> fromLinearToGrayscale(const LinearImage& image) {
  const size_t w = image.getWidth();
  const size_t h = image.getHeight();
...
static LinearImage toLinear(size_t w, size_t h, size_t bpr, const uint8_t* src, PROCESS proc, TRANSFORM transform) {
...
// The "transform" lambda performs an arbitrary float-to-float transformation.
template <typename T, typename PROCESS, typename TRANSFORM>
static LinearImage toLinear(size_t w, size_t h, size_t bpr, const std::unique_ptr<uint8_t[]>& src, PROCESS proc, TRANSFORM transform) {
...
// the "transform" lambda performs an arbitrary float-to-float transformation.
template <typename T, typename PROCESS, typename TRANSFORM>
static LinearImage toLinearWithAlpha(size_t w, size_t h, size_t bpr, const uint8_t* src, PROCESS proc, TRANSFORM transform) {
...
// the "transform" lambda performs an arbitrary float-to-float transformation.
template <typename T, typename PROCESS, typename TRANSFORM>
static LinearImage toLinearWithAlpha(size_t w, size_t h, size_t bpr, const std::unique_ptr<uint8_t[]>& src, PROCESS proc,
                                     TRANSFORM transform) {
...
template <typename T> static LinearImage toLinearWithAlpha(size_t w, size_t h, size_t bpr, const uint8_t* src) {
  LinearImage result(w, h, 4);
...
template <typename T> static LinearImage toLinear(size_t w, size_t h, size_t bpr, const uint8_t* src) {
  LinearImage result(w, h, 3);
