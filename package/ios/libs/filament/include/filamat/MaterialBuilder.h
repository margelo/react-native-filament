...
public:
   ...
   */
  enum class TargetApi : uint8_t { OPENGL = 0x01u, VULKAN = 0x02u, METAL = 0x04u, ALL = OPENGL | VULKAN | METAL };
...
inline constexpr MaterialBuilderBase::TargetApi targetApiFromBackend(filament::backend::Backend backend) noexcept {
...
public:
  ...
  struct PreprocessorDefine {
    ...
    PreprocessorDefine(std::string name, std::string value) : name(std::move(name)), value(std::move(value)) {}
  };
  ...
  MaterialBuilder& parameter(const char* name, UniformType type, ParameterPrecision precision = ParameterPrecision::DEFAULT) noexcept;
  ...
  using is_supported_constant_parameter_t =
      typename std::enable_if<std::is_same<int32_t, T>::value || std::is_same<float, T>::value || std::is_same<bool, T>::value>::type;
  ...
  MaterialBuilder& parameter(const char* name, SamplerType samplerType, SamplerFormat format = SamplerFormat::FLOAT,
                             ParameterPrecision precision = ParameterPrecision::DEFAULT, bool multisample = false) noexcept;
  ...
  MaterialBuilder& output(VariableQualifier qualifier, OutputTarget target, Precision precision, OutputType type, const char* name,
                          int location = -1) noexcept;
...
public:
   ...
   */
  MaterialBuilder& subpass(SubpassType subpassType, SamplerFormat format, ParameterPrecision precision, const char* name) noexcept;
  MaterialBuilder& subpass(SubpassType subpassType, SamplerFormat format, const char* name) noexcept;
  MaterialBuilder& subpass(SubpassType subpassType, ParameterPrecision precision, const char* name) noexcept;
  ...
  struct Parameter {
    ...
    bool multisample;
    enum { INVALID, UNIFORM, SAMPLER, SUBPASS } parameterType;
    ...
    bool isSubpass() const {
      return parameterType == SUBPASS;
    }
  };

  struct Output {
    Output() noexcept = default;
    Output(const char* outputName, VariableQualifier qualifier, OutputTarget target, Precision precision, OutputType type,
           int location) noexcept
        : name(outputName), qualifier(qualifier), target(target), precision(precision), type(type), location(location) {}
  ...
  static_assert(MAX_COLOR_OUTPUT == 8, "When updating MRT::MAX_SUPPORTED_RENDER_TARGET_COUNT, manually update post_process_inputs.fs"
                                       " and post_process.fs");
  ...
  std::string peek(filament::backend::ShaderStage type, const CodeGenParams& params, const PropertyList& properties) noexcept;
  ...
  // returns the number of parameters declared in this material
  uint8_t getSubpassCount() const noexcept {
    return mSubpassCount;
  }

  // returns a list of at least getParameterCount() parameters
  const SubpassList& getSubPasses() const noexcept {
    return mSubpasses;
  }

  filament::UserVariantFilterMask getVariantFilter() const {
...
private:
  ...
  bool findProperties(filament::backend::ShaderStage type, MaterialBuilder::PropertyList& allProperties,
                      CodeGenParams const& semanticCodeGenParams) noexcept;

  bool runSemanticAnalysis(MaterialInfo* inOutInfo, CodeGenParams const& semanticCodeGenParams) noexcept;
  ...
  bool generateShaders(utils::JobSystem& jobSystem, const std::vector<filamat::Variant>& variants, ChunkContainer& container,
  ...
  bool isLit() const noexcept {
    return mShading != filament::Shading::UNLIT;
  }
...
template <> struct utils::EnableBitMaskOperators<filamat::MaterialBuilder::TargetApi> : public std::true_type {};
