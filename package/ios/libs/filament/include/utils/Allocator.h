...
namespace pointermath {
  ...
  template <typename P> static inline P* align(P* p, size_t alignment, size_t offset) noexcept {
...
public:
  ...
  template <typename AREA> explicit LinearAllocator(const AREA& area) : LinearAllocator(area.begin(), area.end()) {}
  ...
  void const* base() const noexcept {
    return mBegin;
  }
...
protected:
  ...
  void const* end() const UTILS_RESTRICT noexcept {
    return pointermath::add(mBegin, mSize);
  }
  ...
  void const* current() const UTILS_RESTRICT noexcept {
    return pointermath::add(mBegin, mCur);
  }
...
public:
  LinearAllocatorWithFallback(void* begin, void* end) noexcept : LinearAllocator(begin, end) {}

  template <typename AREA> explicit LinearAllocatorWithFallback(const AREA& area) : LinearAllocatorWithFallback(area.begin(), area.end()) {}
...
template <size_t ELEMENT_SIZE, size_t ALIGNMENT = alignof(std::max_align_t), size_t OFFSET = 0, typename FREELIST = FreeList>
class PoolAllocator {
  static_assert(ELEMENT_SIZE >= sizeof(typename FREELIST::Node), "ELEMENT_SIZE must accommodate at least a FreeList::Node");
...
public:
  ...
  PoolAllocator(void* begin, void* end) noexcept : mFreeList(begin, end, ELEMENT_SIZE, ALIGNMENT, OFFSET) {}

  PoolAllocator(void* begin, size_t size) noexcept : mFreeList(begin, static_cast<char*>(begin) + size, ELEMENT_SIZE, ALIGNMENT, OFFSET) {}

  template <typename AREA> explicit PoolAllocator(const AREA& area) noexcept : PoolAllocator(area.begin(), area.end()) {}
...
using ObjectPoolAllocator = PoolAllocator<sizeof(T), UTILS_MAX(alignof(FreeList), alignof(T)), OFFSET>;

template <typename T, size_t OFFSET = 0>
using ThreadSafeObjectPoolAllocator = PoolAllocator<sizeof(T), UTILS_MAX(alignof(FreeList), alignof(T)), OFFSET, AtomicFreeList>;
...
namespace TrackingPolicy {
  ...
  struct HighWatermark {
    ...
    HighWatermark(const char* name, void* base, size_t size) noexcept : mName(name), mBase(base), mSize(uint32_t(size)) {}
  ...
  struct Debug {
    ...
    Debug(const char* name, void* base, size_t size) noexcept : mName(name), mBase(base), mSize(uint32_t(size)) {}
    void onAlloc(void* p, size_t size, size_t alignment, size_t extra) noexcept;
    void onFree(void* p, size_t size) noexcept;
    void onReset() noexcept;
    void onRewind(void* addr) noexcept;
  ...
  struct DebugAndHighWatermark : protected HighWatermark, protected Debug {
    ...
    DebugAndHighWatermark(const char* name, void* base, size_t size) noexcept : HighWatermark(name, base, size), Debug(name, base, size) {}
    void onAlloc(void* p, size_t size, size_t alignment, size_t extra) noexcept {
...
template <typename AllocatorPolicy, typename LockingPolicy, typename TrackingPolicy = TrackingPolicy::Untracked,
...
public:
  ...
  Arena(const char* name, size_t size, ARGS&&... args)
      : mArenaName(name), mArea(size), mAllocator(mArea, std::forward<ARGS>(args)...), mListener(name, mArea.data(), mArea.size()) {}

  template <typename... ARGS>
  Arena(const char* name, AreaPolicy&& area, ARGS&&... args)
      : mArenaName(name), mArea(std::forward<AreaPolicy>(area)), mAllocator(mArea, std::forward<ARGS>(args)...),
        mListener(name, mArea.data(), mArea.size()) {}

  template <typename... ARGS> void* alloc(size_t size, size_t alignment, size_t extra, ARGS&&... args) noexcept {
  ...
  template <typename... ARGS> void free(void* p, size_t size, ARGS&&... args) noexcept {
    if (p) {
      std::lock_guard<LockingPolicy> guard(mLock);
  ...
  template <typename T, size_t ALIGN = alignof(T), typename... ARGS> T* make(ARGS&&... args) noexcept {
  ...
  TrackingPolicy& getListener() noexcept {
    return mListener;
  }
  TrackingPolicy const& getListener() const noexcept {
    return mListener;
  }
...
public:
  ...
  explicit ArenaScope(ARENA& allocator) : mArena(allocator), mRewind(allocator.getCurrent()) {}
  ...
  template <typename T> T* allocate(size_t size, size_t alignment = alignof(T), size_t extra = 0) noexcept {
    return mArena.template alloc<T>(size, alignment, extra);
  }

  // use with caution
  ARENA& getArena() noexcept {
    return mArena;
  }
...
public:
  ...
  template <typename U> explicit STLAllocator(STLAllocator<U, ARENA> const& rhs) : mArena(rhs.mArena) {}
  ...
  template <typename U, typename A> bool operator==(const STLAllocator<U, A>& rhs) const noexcept {
