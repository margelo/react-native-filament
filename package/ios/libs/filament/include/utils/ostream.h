...
public:
  ...
  ostream& operator<<(ostream& (*f)(ostream&)) noexcept {
    return f(*this);
  }
...
private:
  ...
  enum type { SHORT, USHORT, CHAR, UCHAR, INT, UINT, LONG, ULONG, LONG_LONG, ULONG_LONG, FLOAT, DOUBLE, LONG_DOUBLE };
...
template <template <typename T> class VECTOR, typename T> inline ostream& operator<<(ostream& stream, const VECTOR<T>& v) {
  stream << "< ";
...
inline ostream& hex(ostream& s) noexcept {
  return s.hex();
}
inline ostream& dec(ostream& s) noexcept {
  return s.dec();
}
inline ostream& endl(ostream& s) noexcept {
  return flush(s << '\n');
}
