 ...
 *
 */
template <typename TYPE> class STLAlignedAllocator {
...
public:
  ...
  template <typename T> inline explicit STLAlignedAllocator(const STLAlignedAllocator<T>&) noexcept {}
  ...
  template <typename T> bool operator==(const STLAlignedAllocator<T>&) const noexcept {
    return true;
  }
