...
template <typename Allocator, typename... Elements> class StructureOfArraysBase {
  // number of elements
...
public:
  ...
  template <size_t N> using TypeAt = typename std::tuple_element_t<N, Structure>;

  // Number of arrays
  static constexpr size_t getArrayCount() noexcept {
    return kArrayCount;
  }
  ...
  class IteratorValueRef {
    ...
    template <size_t... Is> IteratorValueRef& assign(IteratorValue const& rhs, std::index_sequence<Is...>);

    // assigns a value_type to a reference (i.e. assigns to what's pointed to by the reference)
    template <size_t... Is> IteratorValueRef& assign(IteratorValue&& rhs, std::index_sequence<Is...>) noexcept;
  ...
  public:
    ...
    template <size_t I> TypeAt<I> const& get() const {
      return soa->elementAt<I>(index);
    }
    template <size_t I> TypeAt<I>& get() {
      return soa->elementAt<I>(index);
    }
  };
  ...
  class IteratorValue {
    ...
    template <size_t... Is> static Type init(IteratorValueRef const& rhs, std::index_sequence<Is...>) {
      return Type{rhs.soa->template elementAt<Is>(rhs.index)...};
    }

    template <size_t... Is> static Type init(IteratorValueRef&& rhs, std::index_sequence<Is...>) noexcept {
  ...
  public:
    ...
    IteratorValue(IteratorValueRef const& rhs) : elements(init(rhs, std::make_index_sequence<kArrayCount>())) {}
    IteratorValue(IteratorValueRef&& rhs) noexcept : elements(init(rhs, std::make_index_sequence<kArrayCount>())) {}
    IteratorValue& operator=(IteratorValueRef const& rhs) {
      return operator=(IteratorValue(rhs));
    }
    IteratorValue& operator=(IteratorValueRef&& rhs) noexcept {
      return operator=(IteratorValue(rhs));
    }
   ...
   */
  template <typename CVQualifiedSOAPointer> class Iterator {
  ...
  public:
    ...
    template <size_t I> TypeAt<I> const& get() const {
      return soa->template elementAt<I>(index);
    }
    template <size_t I> TypeAt<I>& get() {
      return soa->template elementAt<I>(index);
    }
  ...
  template <std::size_t... Indices> struct ElementIndices {};

  template <std::size_t N, std::size_t... Indices> struct BuildElementIndices : BuildElementIndices<N - 1, N - 1, Indices...> {};

  template <std::size_t... Indices> struct BuildElementIndices<0, Indices...> : ElementIndices<Indices...> {};

  template <std::size_t... Indices> void push_back_unsafe(Structure&& args, ElementIndices<Indices...>) {
    size_t last = mSize++;
    // Fold expression on the comma operator
    ([&] { new (std::get<Indices>(mArrays) + last) Elements{std::get<Indices>(args)}; }(), ...);
  }

  template <std::size_t... Indices> void push_back_unsafe(Elements const&... args, ElementIndices<Indices...>) {
    size_t last = mSize++;
    // Fold expression on the comma operator
    ([&] { new (std::get<Indices>(mArrays) + last) Elements{args}; }(), ...);
  }

  template <std::size_t... Indices> void push_back_unsafe(Elements&&... args, ElementIndices<Indices...>) {
    size_t last = mSize++;
    // Fold expression on the comma operator
  ...
  template <typename F, typename... ARGS> void forEach(F&& f, ARGS&&... args) {
    for_each(mArrays, [&](size_t, auto* p) { f(p, std::forward<ARGS>(args)...); });
  }

  // return a pointer to the first element of the ElementIndex]th array
  template <size_t ElementIndex> TypeAt<ElementIndex>* data() noexcept {
    return std::get<ElementIndex>(mArrays);
  }

  template <size_t ElementIndex> TypeAt<ElementIndex> const* data() const noexcept {
    return std::get<ElementIndex>(mArrays);
  }

  template <size_t ElementIndex> TypeAt<ElementIndex>* begin() noexcept {
    return std::get<ElementIndex>(mArrays);
  }

  template <size_t ElementIndex> TypeAt<ElementIndex> const* begin() const noexcept {
    return std::get<ElementIndex>(mArrays);
  }

  template <size_t ElementIndex> TypeAt<ElementIndex>* end() noexcept {
    return std::get<ElementIndex>(mArrays) + size();
  }

  template <size_t ElementIndex> TypeAt<ElementIndex> const* end() const noexcept {
    return std::get<ElementIndex>(mArrays) + size();
  }

  template <size_t ElementIndex> Slice<TypeAt<ElementIndex>> slice() noexcept {
    return {begin<ElementIndex>(), end<ElementIndex>()};
  }

  template <size_t ElementIndex> Slice<const TypeAt<ElementIndex>> slice() const noexcept {
    return {begin<ElementIndex>(), end<ElementIndex>()};
  }

  // return a reference to the index'th element of the ElementIndex'th array
  template <size_t ElementIndex> TypeAt<ElementIndex>& elementAt(size_t index) noexcept {
    return data<ElementIndex>()[index];
  }

  template <size_t ElementIndex> TypeAt<ElementIndex> const& elementAt(size_t index) const noexcept {
    return data<ElementIndex>()[index];
  }

  // return a reference to the last element of the ElementIndex'th array
  template <size_t ElementIndex> TypeAt<ElementIndex>& back() noexcept {
    return data<ElementIndex>()[size() - 1];
  }

  template <size_t ElementIndex> TypeAt<ElementIndex> const& back() const noexcept {
    return data<ElementIndex>()[size() - 1];
  }

  template <size_t E, typename IndexType = uint32_t> struct Field {
    SoA& soa;
    IndexType i;
    ...
    // calling the field
    template <typename... ARGS> UTILS_ALWAYS_INLINE decltype(auto) operator()(ARGS&&... args) noexcept {
      return soa.elementAt<E>(i)(std::forward<ARGS>(args)...);
    }
    template <typename... ARGS> UTILS_ALWAYS_INLINE decltype(auto) operator()(ARGS&&... args) const noexcept {
      return soa.elementAt<E>(i)(std::forward<ARGS>(args)...);
    }
  };
...
private:
  ...
  template <std::size_t I = 0, typename FuncT, typename... Tp>
  inline typename std::enable_if<I == sizeof...(Tp), void>::type for_each_index(std::tuple<Tp...>&, FuncT) {}

  template <std::size_t I = 0, typename FuncT, typename... Tp>
      inline typename std::enable_if < I<sizeof...(Tp), void>::type for_each_index(std::tuple<Tp...>& t, FuncT f) {
  ...
  void move_each(void* buffer, size_t capacity) noexcept {
    ...
    if (mSize) {
      ...
      forEach([buffer, &index, &offsets, size](auto p) {
        ...
        T* UTILS_RESTRICT const arrayPointer = reinterpret_cast<T*>(uintptr_t(b) + offsets[index]);
...
inline typename StructureOfArraysBase<Allocator, Elements...>::IteratorValueRef&
StructureOfArraysBase<Allocator, Elements...>::IteratorValueRef::operator=(StructureOfArraysBase::IteratorValueRef const& rhs) {
  return operator=(IteratorValue(rhs));
}

template <typename Allocator, typename... Elements>
inline typename StructureOfArraysBase<Allocator, Elements...>::IteratorValueRef&
StructureOfArraysBase<Allocator, Elements...>::IteratorValueRef::operator=(StructureOfArraysBase::IteratorValueRef&& rhs) noexcept {
  return operator=(IteratorValue(rhs));
}
...
template <size_t... Is>
inline typename StructureOfArraysBase<Allocator, Elements...>::IteratorValueRef&
StructureOfArraysBase<Allocator, Elements...>::IteratorValueRef::assign(StructureOfArraysBase::IteratorValue const& rhs,
...
template <size_t... Is>
inline typename StructureOfArraysBase<Allocator, Elements...>::IteratorValueRef&
StructureOfArraysBase<Allocator, Elements...>::IteratorValueRef::assign(StructureOfArraysBase::IteratorValue&& rhs,
                                                                        std::index_sequence<Is...>) noexcept {
  // implements IteratorValueRef& IteratorValueRef::operator=(IteratorValue&& rhs) noexcept
  auto UTILS_UNUSED l = {(soa->elementAt<Is>(index) = std::move(std::get<Is>(rhs.elements)), 0)...};
  return *this;
}

template <typename... Elements> using StructureOfArrays = StructureOfArraysBase<HeapArena<>, Elements...>;
