...
template <size_t N> using StringLiteral = const char[N];
...
public:
  ...
  const_pointer c_str_safe() const noexcept {
    return mData ? c_str() : "";
  }
  ...
  CString& insert(size_type pos, const CString& str) noexcept {
...
template <typename T> CString to_string(T value) noexcept;

// ------------------------------------------------------------------------------------------------

template <size_t N> class UTILS_PUBLIC FixedSizeString {
public:
  using value_type = char;
  using pointer = value_type*;
  using const_pointer = const value_type*;
