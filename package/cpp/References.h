...
public:
     ...
     * When the resulting `shared_ptr` is deleted, `cleanup` will be called with the reference to T, and the caller is expected to properly clean up T.
     * @param value The raw reference to T that will be adopted.
     * @param cleanup The function to run when the shared_ptr's ref count reaches zero. The caller is expected to delete the reference to T here.
    ...
    static std::shared_ptr<T> adoptEngineRef(std::shared_ptr<filament::Engine> engine, T* value, CleanupEngineRefFunction cleanup) {
        return adoptRef(value, [engine, cleanup = std::move(cleanup)] (T* ref) {
            cleanup(engine, ref);
        });
    }
