...
void PromiseFactory::install(jsi::Runtime& runtime, const std::shared_ptr<react::CallInvoker>& callInvoker) {
  ...
  runtime.global().setProperty(runtime, GLOBAL_PROMISE_FACTORY_CALL_INVOKER_HOLDER_NAME, callInvokerHolder);
}
...
jsi::Value PromiseFactory::createPromise(jsi::Runtime& runtime, RunPromise&& run) {
  ...
  if (!runtime.global().hasProperty(runtime, GLOBAL_PROMISE_FACTORY_CALL_INVOKER_HOLDER_NAME)) {
    throw jsi::JSError(runtime, "Failed to create Promise - the global CallInvoker holder does not exist! "
                                ...
                                "(global." +
                                    std::string(GLOBAL_PROMISE_FACTORY_CALL_INVOKER_HOLDER_NAME) + " == undefined)");
  }
#endif
  jsi::Object callInvokerHolder = runtime.global().getPropertyAsObject(runtime, GLOBAL_PROMISE_FACTORY_CALL_INVOKER_HOLDER_NAME);
  ...
  if (!callInvokerHolder.hasNativeState(runtime)) {
    throw jsi::JSError(runtime, "Failed to create Promise - the global CallInvoker holder "
                                "(global." +
                                    std::string(GLOBAL_PROMISE_FACTORY_CALL_INVOKER_HOLDER_NAME) +
                                    ") does not "
  ...
  return Promise::createPromise(runtime,
                                [callInvokerState, run = std::move(run)](jsi::Runtime& runtime, const std::shared_ptr<Promise>& promise) {
