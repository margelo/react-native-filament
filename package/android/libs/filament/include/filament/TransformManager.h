 ...
 *
 * A Transform component gives an entity a position and orientation in space in
 * the coordinate space of its parent transform. The TransformManager takes care
 * of computing the world-space transform of each component (i.e. its transform
 * relative to the root).
 *
 * Creation and destruction
 * ========================
 *
 * A transform component is created using TransformManager::create() and
 * destroyed by calling TransformManager::destroy().
 *
 * ~~~~~~~~~~~{.cpp}
 *  filament::Engine* engine = filament::Engine::create();
 *  utils::Entity object = utils::EntityManager.get().create();
 *
 *  auto& tcm = engine->createTransformManager();
 *
...
public:
  ...
  /**
   * Gets an Instance representing the transform component associated with the
   * given Entity.
   * @param e An Entity.
   * @return An Instance object, which represents the transform component
   * associated with the Entity e.
   ...
   * @return A list, in no particular order, of all the entities managed by this
   * manager.
   */
   ...
   *
   * When accurate translation mode is active, the translation component of all
   * transforms is maintained at double precision. This is only useful if the
   * mat4 version of setTransform() is used, as well as getTransformAccurate().
   *
   * @param enable true to enable the accurate translation mode, false to
   * disable.
   *
   ...
   * @param parent            The Instance of the parent transform, or
   * Instance{} if no parent.
   * @param localTransform    The transform to initialize the transform
   * component with. This is always relative to the parent.
   *
   * If this component already exists on the given entity, it is first destroyed
   * as if destroy(utils::Entity e) was called.
   *
   * @see destroy()
   */
  ...
  void create(utils::Entity entity, Instance parent,
              const math::mat4 &localTransform);           //!< \overload
   ...
   * @param e An entity.
   *
   * @note If this transform had children, these are orphaned, which means their
   * local transform becomes a world transform. Usually it's nonsensical. It's
   * recommended to make sure that a destroyed transform doesn't have children.
   *
   * @see create()
   */
   ...
   * @attention It is an error to re-parent an entity to a descendant and will
  ...
  /**
   * Returns the parent of a transform component, or the null entity if it is a
   * root.
   ...
   *
   * @param i The instance of the transform component to query.
   * @param children Pointer to array-of-Entity. The array must have at least
   * "count" elements.
   * @param count The maximum number of children to retrieve.
   * @return The number of children written to the pointer.
   */
  size_t getChildren(Instance i, utils::Entity *UTILS_NONNULL children,
  ...
  /**
   * Returns an undreferencable iterator representing the end of the children
   * list
   *
   * @param parent Instance of the parent
   ...
   * @param ci              The instance of the transform component to set the
   * local transform to.
   * @param localTransform  The local transform (i.e. relative to the parent).
   * @see getTransform()
   * @attention This operation can be slow if the hierarchy of transform is too
   * deep, and this will be particularly bad when updating a lot of transforms.
   * In that case, consider using openLocalTransformTransaction() /
   * commitLocalTransformTransaction().
   */
  void setTransform(Instance ci, const math::mat4f &localTransform) noexcept;

  /**
   * Sets a local transform of a transform component and keeps double precision
   * translation. All other values of the transform are stored at single
   * precision.
   * @param ci              The instance of the transform component to set the
   * local transform to.
   * @param localTransform  The local transform (i.e. relative to the parent).
   ...
   * @param ci The instance of the transform component to query the local
   * transform from.
   * @return The local transform of the component (i.e. relative to the parent).
   * This always returns the value set by setTransform().
   * @see setTransform()
   */
   ...
   * @param ci The instance of the transform component to query the world
   * transform from.
   * @return The world transform of the component (i.e. relative to the root).
   * This is the composition of this component's local transform with its
  ...
  /**
   * Opens a local transform transaction. During a transaction,
   * getWorldTransform() can return an invalid transform until
   * commitLocalTransformTransaction() is called. However, setTransform() will
   * perform significantly better and in constant time.
   *
   * This is useful when updating many transforms and the transform hierarchy is
   * deep (say more than 4 or 5 levels).
   *
  ...
  /**
   * Commits the currently open local transform transaction. When this returns,
   * calls to getWorldTransform() will return the proper value.
   *
   * @attention failing to call this method when done updating the local
   * transform will cause a lot of rendering problems. The system never closes
