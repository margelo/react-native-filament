...
JFilamentRecorder::JFilamentRecorder(
    ...
    std::shared_ptr<Dispatcher> rendererDispatcher, int width, int height,
    int fps, double bitRate)
    : FilamentRecorder(rendererDispatcher, width, height, fps, bitRate),
...
void JFilamentRecorder::registerNatives() {
  registerHybrid({
      ...
      makeNativeMethod("onReadyForMoreData",
                       JFilamentRecorder::onReadyForMoreDataJava),
  });
}
...
jni::local_ref<JDispatcher::javaobject>
JFilamentRecorder::getRecorderDispatcher() {
  static const auto method =
      javaClassLocal()->getMethod<JDispatcher::javaobject()>(
          "getRecorderDispatcher");
...
void *JFilamentRecorder::getNativeWindow() {
  if (_nativeWindow == nullptr) {
    static const auto method =
        javaClassLocal()->getMethod<jobject()>("getNativeWindow");
    jni::local_ref<jobject> nativeWindow = method(_javaPart);
    _nativeWindow = ANativeWindow_fromSurface(jni::Environment::current(),
                                              nativeWindow.get());
  }
...
std::future<void> JFilamentRecorder::startRecording() {
  ...
  return dispatcher->cthis()->runAsyncAwaitable<void>([self]() {
    static const auto method =
        self->javaClassLocal()->getMethod<void()>("startRecording");
...
std::future<std::string> JFilamentRecorder::stopRecording() {
  ...
  return dispatcher->cthis()->runAsyncAwaitable<std::string>([self]() {
    static const auto method =
        self->javaClassLocal()->getMethod<void()>("stopRecording");
    method(self->_javaPart);
...
std::string JFilamentRecorder::getOutputFile() {
  static const auto method =
      javaClassLocal()->getMethod<std::string()>("getOutputFile");
...
bool JFilamentRecorder::getIsRecording() {
  static const auto method =
      javaClassLocal()->getMethod<jboolean()>("getIsRecording");
  return method(_javaPart);
}

jni::local_ref<JFilamentRecorder::jhybriddata> JFilamentRecorder::initHybrid(
    jni::alias_ref<JFilamentRecorder::jhybridobject> jThis,
    jni::alias_ref<JDispatcher::javaobject> rendererDispatcher, int width,
    int height, int fps, double bitRate) {
  __android_log_write(ANDROID_LOG_INFO, TAG,
                      "Initializing JFilamentRecorder...");
  jni::global_ref<JDispatcher::javaobject> globalRef =
      jni::make_global(rendererDispatcher);
